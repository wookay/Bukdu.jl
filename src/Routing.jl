...
route(args...) = Route(System.MissingController, System.not_found, Dict{Symbol,DataType}(), Vector{Pair{String,Any}}())
...
function add_route(verb, url::String, C::Type{<:ApplicationController}, action, param_types::Dict{Symbol,DataType}) # throw AbstractControllerError
    ...
    @eval route(::Val{Symbol($method)}, $(vals...)) = Route($C, $action, $param_types, Vector{Pair{String,Any}}($(path_params...)))
    routing_tables = vcat(store[:routing_tables],
        Naming.verb_name(verb), url, nameof(C), nameof(action))
...
function empty!()
    ...
    @eval route(args...) = Route(System.MissingController, System.not_found, Dict{Symbol,DataType}(), Vector{Pair{String,Any}}())
end
